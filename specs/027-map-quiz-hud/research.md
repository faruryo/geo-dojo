...
| hold | 1000ms | 2500ms |
| transition | 320ms（合計 1.32s、FR-021 の 0.8〜1.2s＋遷移に収まる） | 0ms（即時） |
| 定常前の下端 | 通常サイズ | 帯 64px・文字 24px |
...
参考: 変更後の想定値は幅 375px（全幅）× 高さ 724px（812 − 上端 44 − 下端 44）＝ 271,500 px² で、
ベースライン比 162%。余裕をもって達成ラインを超える見込み。
...
最長形（spec FR-026 の3形のうち3番目）:

```
大和町 （正解: 宮城県: たいわちょう / 神奈川県: やまとまち）
```
...
暫定値は 72px（2行分）とし、tasks.md に「375px 実機幅で最長形を描画して確定する」検証タスクを置く。
