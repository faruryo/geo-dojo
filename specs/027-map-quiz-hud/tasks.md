...
### Issue #90: 地図タップの取りこぼし（2026-09-12 ローカル検証）

- [x] `JapanMap` の地図要素を親更新・選択・正誤表示で再マウントしないよう修正。
- [x] ドラッグ後の遅延 click を最大1回抑止し、300ms タイムアウトまたは続く非ドラッグタップの `pointerup` で解除する（#98 / #99）。
...
- [x] 実機 iPhone（Safari）で都道府県クイズの1タップ選択・hover 張り付き解消を確認（2026-09-12）。
...
- [x] T045 [P] `lib/quiz/countdown-pulse.ts` に純粋関数 `shouldShowMapPulse(secondsLeft: number, feedback: FeedbackState): boolean` と `getMapPulseType(secondsLeft: number): MapPulseType` を実装し、単体テスト `__tests__/lib/quiz/countdown-pulse.test.ts` を作成する（条件反転確認済み）。
...
- [x] T047 `components/quiz/hud/map-countdown-pulse.tsx` を作成し、`pointer-events-none`、`key={secondsLeft}` による毎秒再発火、`clip-path` による左下帰属表示（FR-032）除外、`prefers-reduced-motion` による抑止、コーラルレッドエッジを描画する。
- [x] T048 `components/quiz/hud/immersive-quiz-view.tsx` の `Stage` コンテナに `MapCountdownPulse` を配置し、モード D 出題中に `secondsLeft` と `feedback` を連携する。
- [x] T049 `__tests__/components/quiz/map-countdown-pulse.test.tsx` で DOM 再生成（要素同一性チェック）、帰属表示除外（`clip-path`）、非表示・回答後非表示・減速アニメーション設定時の挙動を検証する（条件反転確認済み）。
