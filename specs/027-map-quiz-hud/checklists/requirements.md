...
6. **情報要素の棚卸し表**（FR-040）。正解数と難易度バッジは廃止、残り時間（D）と経過タイム（都道府県 TA）は別要件として書き分け（FR-011 / FR-012）、選択中バッジ列は件数1行に集約（FR-023）。
7. **下端 HUD の高さ固定**（FR-022 / SC-008）。親指の直下でレイアウトが動くことによる次問の誤タップを防ぐ。
...
5. **タイマーは導入表示の完了後に開始**（FR-024）。読んでいる間に制限時間が減らないようにする。
