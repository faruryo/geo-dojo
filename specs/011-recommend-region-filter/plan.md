...
**Testing**: Vitest (`pnpm test`)。フィルタロジック、および `isModeAvailable` のテストを拡張する。  
...
**Constraints**: 既存の推薦 API (`getRecommendation`) や Server Actions を変更せず、フロントエンドおよび出題生成のクエリ連携のみで完結させる。
...
| コーディング規約 | TypeScript strict を順守。フィルタロジックの判定は `lib/quiz/` の純粋関数を拡張しテストで検証。 | ✅ Pass |
...
**Structure Decision**: 既存の Next.js 単一プロジェクトの構造（LIFT原則）を踏襲する。UI コンポーネントおよびクイズプレイ画面の変更のみで実現する。
