...
### Decision 2: クエリパラメータ `prefectures` の新設（※廃止）

> [!WARNING]
> **※仕様変更による廃止:** モードA/Bの解答自明化（例：「宮城県」を選んだときに逆引きで「宮城県」が答えとすぐ分かってしまう問題）を防ぐため、都道府県レベルのフィルタは廃止され、地方レベルのフィルタ（`region`）に一本化されました。以下は初期検討時の内容です。
...
### Decision 3: クライアントサイドフィルタ（`buildQuestions`）の拡張（※廃止）

> [!WARNING]
> **※仕様変更による廃止:** 都道府県フィルタの廃止に伴い、`buildQuestions` での都道府県レベルの絞り込み処理は実装されず、既存の `filterByRegions` を用いた地方レベルのフィルタリングのみが適用されます。
