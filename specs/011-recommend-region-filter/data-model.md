...
クライアントサイドで保持する設定データ構造と、開始時 URL へのシリアライズ形式について定義します。
...
### 2. オーバーライドの適用範囲
地域・モード・難易度の差し替えはシート表示中の明示操作に限定する。LocalStorage（旧キー `geodojo-recommend-region-filters`）への保存・マウント時復元は行わない（024 の自律選定と衝突するため。#107）。
