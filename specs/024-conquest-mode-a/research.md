...
1. 90%未満マスが残っているあいだ: `random() < 0.5` でモード A または D。
2. そのモードで開始できる地方から均等抽選。A は北海道を除く。全難易度が制覇 90% 以上の地方は捨てて引き直す。A の「全市」も北海道除外後の地方集合。
3. 選んだ地方×モードで、90% 未満のうち最も易しい難易度。
4. 抽選対象の全市が 90% 以上なら、1–3 を捨てて A/D の開始できる地方×難易度マスから制覇率最小（同率は均等）。mode・地方・難易度はこのマスで確定し、ステップ3で上書きしない。100% しか無いならその中から均等。開始不能マス（A かつ北海道。最小規則で B は出さない）は除外。
