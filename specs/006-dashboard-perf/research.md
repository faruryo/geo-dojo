...
2026-08-29: 全クエリ一括プリフェッチは `completionTrend` が伸びると当時 8s の安全弁までブロックし、フォールバック後のクライアント取得と重複する。対応は全解除ではなく **タイムアウト 1.5s・制覇率推移のプリフェッチ除外・ファーストビュー並行マウント・下部チャート inView**。SQL 統合は別 PR。
