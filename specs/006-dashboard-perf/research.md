...
### 案3（採用・#66 で部分見直し）: サーバ並列プリフェッチ ＋ TanStack Query ハイドレーション（ハイブリッド）
`page.tsx` を薄い server wrapper にし、ファーストビュー向け read を **認証1回 ＋ `Promise.all`** で取得 → `dehydrate`/`HydrationBoundary` でクライアントへ。各部品はハイドレート済みなら初回フェッチせず即描画。フィルタ変更・手動更新のみ既存取得関数でオンデマンド再取得。

2026-08-29: 全クエリ一括プリフェッチは `completionTrend` が伸びると当時 8s の安全弁までブロックし、フォールバック後のクライアント取得と重複する。対応は全解除ではなく **ファーストビューのみプリフェッチ・下部チャート inView・タイムアウト 2s（残バッチ最遅 1,585ms 超）**。SQL 統合は別 PR。
