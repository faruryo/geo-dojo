...
- 2026-07-14: PR #24 への Gemini Code Assist 指摘（3件）を反映。①`useDueReviewSummary` の `isLoading` 中は表示判定を確定させない（表示→消滅のチラつき防止）、②`onComplete` の `invalidateQueries` を `await` してから `phase='result'` へ遷移（古い残数の一瞬表示を防止）、③quickstart.md 手順5を「ネットワーク全体オフライン」から「残数取得のみブロック」に修正（オフラインだと続行自体の検証が成立しないため）。contracts / data-model.md / tasks.md T012-T013 / quickstart.md 手順5 を更新。
