...
日本の地理を学びたいユーザー向けに、都道府県クイズ・市区町村クイズ（モードA〜D＋難易度フィルター）を提供するPWAを Next.js 15 App Router + Supabase で実装。v0.2.0 でカード/SRS/AI 機能を削除し、クイズ専用アプリに簡素化。

## Technical Context

**Language/Version**: TypeScript / Next.js 15.2.6（App Router, React 19）
**Primary Dependencies**: Drizzle ORM, TanStack Query v5, @serwist/next, @vnedyalk0v/react19-simple-maps, @googlemaps/js-api-loader, Tailwind CSS v4, shadcn/ui
...
| チェック項目 | 状態 | 備考 |
|-------------|------|------|
| クライアント公開 API キーに referrer 制限を設定 | ✅ PASS | `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` に HTTP referrer 制限設定済み |
| サーバー専用キーに `NEXT_PUBLIC_` を付けない | ✅ PASS | `E_STAT_APP_ID`, `SUPABASE_SECRET_KEY` 等 |
