...
- [ ] B022 【UI/UX】地図クイズのフルスクリーン化とHUD（オーバーレイ）UI（GeoGuessr風レイアウト） → **027-map-quiz-hud で spec 策定済み・設計レビュー反映済み（#82）**
  - 仕様: `specs/027-map-quiz-hud/spec.md`（レビューで確定した差分は `checklists/requirements.md` の Notes）
  - スコープ確定: 地図そのものの拡大は Mode D のみ。Mode A・都道府県クイズは全国 SVG 地図の縦横比が固定のため HUD 意匠のみ統一し、地図の拡大は [[B026]] に分離。
...
- [ ] B026 【UI/UX】全国 SVG 地図の縦画面最適化（027 の後続）
  ...
  - 案: 沖縄のインセット化＋縦長 viewBox への再設計 / 投影中心とスケールを container のアスペクト比から算出
  - 影響範囲（要注意）: `lib/quiz/` の自動フォーカス計算（016）、B009 で実装したピンチズームの不動点補正。座標系を変えると両方に波及する。
  - 前提: 027（HUD）が先。027 側は HUD の帯を上下の余白位置に置く設計なので、地図が拡大しても HUD の作り直しは不要。
