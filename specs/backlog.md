...
- [x] B026 【UI/UX】全国 SVG 地図の縦画面最適化 → **027-map-quiz-hud に取り込んで実装完了**
  ...
  - 目的: 縦画面で日本列島が画面高いっぱいに収まるようにし、027 の SC-007（地図面積 130% 以上）を A・都道府県にも適用できる状態にする。
  - 結果: `lib/map/japan-projection.ts` に投影定数を集約し、47都道府県の**本体ポリゴンの外形**が
    収まる範囲へ viewBox を詰めた（400×500 → 400×532、center [138,35] → [136.72,36.44]、
    scale 1000 → 1221）。375×812 で地図面積が 124,605 → 187,125 px²（150%）になり、
    027 の SC-007 を A・都道府県にも適用できるようになった。
  - 沖縄のインセット化は**不採用**。実測すると南へ伸びていた分が失われて縦横比が 0.752 → 0.843 へ
    悪化し、縦画面での描画面積がむしろ減る（187,125 → 166,904 px²）。天気予報の枠は横長画面の
    工夫であり、縦画面では逆に働く。
  - 残る余白（約 225px）は日本の外形（Mercator 上で縦横比 0.988）に由来し、47都道府県を保つ限り
    解消できない。これ以上詰めると沖縄本島か北海道東部が枠外へ出る。
  - 影響範囲の扱い: 自動フォーカス（016）は `calculateFocusTransform` の既定値を投影定数へ寄せて
    単一の情報源にした（片方だけ変えるとフォーカスが描画とずれる）。B009 のピンチズームは
    画面座標系で計算しており viewBox に依存しないため影響なし。
  - 経緯: 当初は 027 の後続として分離していたが、027 だけ入れると余白が 75px → 344px に増えて A・都道府県が明確に悪化するため、027 に取り込んだ。

- [ ] B027 【UI/UX】市区町村クイズモード選択画面のUX改善（導線可視化・おすすめ省スペース化・サンプル誤認防止） → **#87**
  - 概要: 市区町村クイズモード選択画面（`/quiz/municipality`）において、初めて訪れたユーザーが迷わずスムーズにクイズ設定へ進めるよう、導線の改善・画面専有要素の整理・プレビューの誤認防止を実施する。
  - 現状の課題:
    1. **「設定に進む」CTAボタンの埋もれ**: 画面最下部に配置されており、モバイル（375px）では上部のおすすめカード＋モード選択（2×2）＋縦長のプレビュー（~480px）＋出題ルールに押され、画面外（ファーストビュー外）に完全に隠れてスクロールしないと見えない。モード選択タップ時に画面下プレビューが変わるだけで「どう進むのか」が伝わらず、ボタン文言（`{shortLabel}・{longLabel} で設定に進む`）も冗長。
    2. **「今日のおすすめクイズ」によるファーストビュー圧迫**: 画面最上部に大きなカード（`RecommendHeroCard`）が常時展開されており、自分でモードを選びに来たユーザーの画面領域を大きく圧迫している。
    3. **モードプレビュー（サンプル画面）がリアルすぎて本番問題と誤認される**: サンプル表示（`ModePreviewA`〜`D`）が本番クイズUI（問題カード、選択肢ボタン、MiniJapanMap等）と同一の質感で描画されているため、「ここで回答するのか」「もう始まっているのか」と勘違いしてタップしようとしたり戸惑ったりする。
  - 改善案:
    - **設定へ進む導線**: 画面下部フローティングCTAバーの導入（どの位置でもワンタップで設定へ）、またはモードカード直結（タップで即設定、カード内CTAなど）。ボタン文言を「クイズ設定へ進む」「このモードで遊ぶ」など簡潔・直感的に変更。
    - **おすすめカードの省スペース化**: アコーディオン折りたたみ化、または上部/ヘッダーのコンパクトなバナー/ポップアップ（シート呼び出し）への変更。
  ...
  - 設計: [specs/028-mode-select-ux/spec.md](./028-mode-select-ux/spec.md) / [plan.md](./028-mode-select-ux/plan.md)（2026-09-13 実装完了・PR 前）
    ...
    - サンプル誤認防止は `ModePreviewA`〜`D` の中身を触らず、外側のラッパー1枚（破線枠・ラベル・減光・`inert`）で行う。
      `pointer-events-none` だけではキーボードで枠内に入れる（react-simple-maps が各パスに `tabIndex={0}` を固定で付ける）ため `inert` が要る。
...
- [x] B028 【バグ/パフォーマンス】「今日のおすすめクイズ」シート開閉の反応遅延・タップフィードバックの改善（モバイル環境） → **#88・実装完了**
  ...
  - 改善案:
    - `isOpen` をローカルの `useState` で管理し、タップした瞬間に即座にシートの開閉アニメーションを開始させる（オプティミスティック更新）。
...
- [ ] B030 【UI/UX】Mode D（場所当て）の制限時間カウントダウン視認性・警告表現の強化（残り秒数通知音・ゲージ拡大アニメーション） → **#96**
  - 概要: 市区町村クイズの Mode D（場所当て地図タップ）における制限時間（30秒）カウントダウンについて、PC大画面などでゲージや秒数が小さく見落としやすい課題を解消するため、残り時間が少なくなった際の警告表現（SE通知音、一時的な拡大パルスアニメーション、PC向けゲージ視認性向上）を導入する。
  - 現状の課題:
    1. **PC大画面でのゲージ・タイマーの視認性不足**: `TopHud`（`components/quiz/hud/top-hud.tsx`）のタイマー表示は幅 56px（`w-14`）、高さ 4px（`h-1`）、文字サイズ 12px（`text-xs`）と非常にコンパクト。PCなどの大画面では中央の広大な地図領域に視線・意識が集中するため、画面右上端にある細いバーと小さな数字は見落とされやすい。
    2. **残り時間低下時のフィードバック不足**: 残り6秒（20%）以下でバーの色が黄色から赤色（`#ef4444`）に変わるのみで、音声による通知やアニメーションによる強調がない。地図探索に夢中になっていると時間の経過に気づかず、突然のタイムアウト（不正解扱い）でユーザー体験を損ねる。
  - 改善案:
    - **音による警告フィードバック（SE）**: 残り5秒以下（5, 4, 3, 2, 1秒）の各秒で、短く控えめな電子音（tick/warning音）を Web Audio API（`lib/quiz/sound-effects.ts`）で動的生成して再生。ミュート設定（`isSoundMuted()`）を尊重。
    - **視覚的フィードバック・アニメーション**: 残り5秒以下で秒数テキストおよびゲージが 1 秒ごとに一瞬ポンと拡大するパルスアニメーション（scale: 1.2〜1.3 → 1.0）を付与。タイマー全体の背景や境界線に薄い赤色の発光（glow）や点滅を追加。
    - **PC画面での視認性向上**: PC/タブレット画面（md以上等）ではゲージの幅・高さを一回り大きく（例: `w-14` → `md:w-24`, `h-1` → `md:h-1.5`）、フォントサイズも `text-xs` → `md:text-sm` に拡大して視界に入りやすくする。
    - **アクセシビリティ配慮**: `prefers-reduced-motion` 有効時は拡大縮小・激しい点滅を抑止し、色の変化や穏やかなフェードにとどめる。
  - 該当ファイル: `components/quiz/hud/top-hud.tsx`, `components/quiz/use-quiz-timer.ts`, `lib/quiz/sound-effects.ts`
...
- [ ] B031 【UI/UX】Mode A（県当て）PC環境での解答ボタン操作性改善（GeoGuessr風Spaceキー即時確定・ボタンの地図直下センタリング） → **#97**
  ...
  - 現状の課題:
    ...
    2. **キーボード操作の未対応**: キーボードショートカットが存在せず、PC環境でもマウス操作のみに依存している。
  - 改善案:
    - **キーボードショートカットによる即時解答（Space / Enter）**: 必要件数の都道府県が選択され、「解答する」ボタンが有効（`canSubmit === true`）な状態で `Space` または `Enter` キーを押すと即座に解答を確定（`handleModeASubmit`）。未選択時や件数不足時（「あと 1 か所」等）はキーを押しても誤爆しないようガード。
